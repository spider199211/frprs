...
- SUDP/XTCP/P2P：已实现 XTCP/SUDP 直连优先数据面、SUDP 直连确认/未确认回退、服务端中继 fallback 和 NAT 控制器基础；更完整的 NAT 打洞协商、候选优选和复杂 NAT 场景仍待实现。
...
- SUDP/XTCP/P2P: XTCP/SUDP direct-first data paths, SUDP direct confirmation/unconfirmed fallback, server-relay fallback, and the NAT controller foundation are implemented; fuller NAT-hole negotiation, candidate preference, and complex NAT scenarios remain.
