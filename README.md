...
- SUDP 私有 visitor 协议，支持 `sk` 密钥校验、`group`/`groupKey` 分组负载均衡、peer 可达时按代理名或分组名直连 UDP 转发、probe 选路、短时 probe 重试、带 TTL 的直连 peer 确认和服务端中继 fallback
...
- SUDP/XTCP/P2P：已实现 XTCP/SUDP 直连优先数据面、SUDP 私有服务分组负载均衡、XTCP/SUDP 按分组名直连注册与探测、XTCP/SUDP owner 候选周期刷新、NAT 控制器同一事务多 peer 候选保留、按 peer 过期清理与轮询、XTCP/SUDP waiting 后短暂等待异步匹配、SUDP probe 选路与短时重试、SUDP owner 主动 punch 短时重试、SUDP 带 TTL 的直连 peer 确认/未确认回退、服务端中继 fallback、NAT 双端匹配通知、客户端带 TTL 的异步通知缓存、候选优选和 XTCP 候选竞速；更完整的复杂 NAT 场景仍待验证和完善。
...
- SUDP private visitor protocol with `sk` authentication, `group`/`groupKey` load balancing, direct UDP forwarding by proxy name or group name when the peer is reachable, probe-based path selection, short probe retries, TTL-capped direct peer confirmation, and server-relay fallback
- NAT hole punching controller, multiple peers per transaction with per-peer expiry, control-channel candidate exchange, two-sided match notification, client-side TTL-capped async notification caching, periodic XTCP/SUDP owner candidate refresh, short XTCP/SUDP wait for async matches after `waiting`, XTCP/SUDP group direct registration, SUDP owner-side active punching with short retries, candidate preference, XTCP candidate racing, and SUDP candidate probing/retry
