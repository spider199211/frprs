...
- SUDP 私有 visitor 协议，支持 `sk` 密钥校验、`group`/`groupKey` 分组负载均衡、peer 可达时按代理名或分组名直连 UDP 转发、probe 选路、短时 probe 重试、有界 pending 队列、带 TTL 的响应确认直连 peer、未确认失败短 TTL 跳过、owner 响应 peer 刷新和服务端中继 fallback
- NAT 打洞控制器、同一事务多 peer 候选保留、按 peer 过期清理与客户端断线主动剪枝、控制通道候选地址交换、双端匹配通知、客户端带 TTL 的异步通知缓存、XTCP/SUDP owner 候选周期刷新、XTCP/SUDP waiting 后短暂等待异步匹配、XTCP/SUDP 分组直连注册、XTCP owner 本地服务确认后接受直连、SUDP owner 主动 punch 短时重试、不可用直连候选过滤、按地址可达性优先的候选优选、XTCP 候选竞速、XTCP 失败候选短 TTL 跳过和 SUDP 候选探测/重试/失败短 TTL 跳过
...
- SUDP/XTCP/P2P：已实现 XTCP/SUDP 直连优先数据面、SUDP 私有服务分组负载均衡、XTCP/SUDP 按分组名直连注册与探测、XTCP/SUDP owner 候选周期刷新、NAT 控制器同一事务多 peer 候选保留、按 peer 过期清理与轮询、XTCP/SUDP waiting 后短暂等待异步匹配、XTCP owner 本地服务确认后接受直连、SUDP probe 选路与短时重试、SUDP direct 有界 pending 队列、SUDP owner 主动 punch 短时重试、SUDP owner 响应 peer 刷新、SUDP 带 TTL 的响应确认直连 peer/未确认回退/失败短 TTL 跳过、服务端中继 fallback、NAT 双端匹配通知、客户端带 TTL 的异步通知缓存、不可用直连候选过滤、按地址可达性优先的候选优选、XTCP 候选竞速和失败候选短 TTL 跳过；更完整的复杂 NAT 场景仍待验证和完善。
