...
- NAT hole punching controller, multiple peers per transaction with per-peer expiry, control-channel candidate exchange, two-sided match notification, client-side TTL-capped async notification caching, periodic XTCP/SUDP owner candidate refresh, short XTCP/SUDP wait for async matches after `waiting`, short TTL deduplication for repeated waiting lookups, XTCP/SUDP group direct registration, XTCP owner-side local-service validation before direct acceptance, SUDP owner-side active punching with short retries, unusable direct-candidate filtering, reachability-first candidate preference, XTCP candidate racing with slower-candidate cancellation, TTL-capped successful XTCP direct peer reuse, short TTL skip for failed XTCP candidates, and SUDP candidate probing/retry plus short TTL skip and retry suppression after failed unconfirmed direct sessions
