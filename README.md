...
- SUDP/XTCP/P2P：已实现 XTCP/SUDP 直连优先数据面、SUDP 私有服务分组负载均衡、XTCP/SUDP 按分组名直连注册与探测、XTCP/SUDP owner 候选周期刷新、NAT 控制器同一事务多 peer 候选保留、按 peer 过期清理与轮询、XTCP/SUDP waiting 后短暂等待异步匹配、SUDP probe 选路与短时重试、SUDP owner 主动 punch 短时重试、SUDP owner 响应 peer 刷新、SUDP 带 TTL 的直连 peer 确认/未确认回退、服务端中继 fallback、NAT 双端匹配通知、客户端带 TTL 的异步通知缓存、候选优选和 XTCP 候选竞速；更完整的复杂 NAT 场景仍待验证和完善。
