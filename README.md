...
- STCP/XTCP 私有 visitor 协议，支持 `sk` 密钥校验和本地 visitor 监听；XTCP 可在 peer 可达且握手响应代理名匹配时直连，失败时回退服务端中继
- STCP/XTCP `group`/`groupKey` 私有服务分组负载均衡，visitor 可通过分组名轮询访问后端
- SUDP 私有 visitor 协议，支持 `sk` 密钥校验、`group`/`groupKey` 分组负载均衡、peer 可达时按代理名或分组名直连 UDP 转发、probe 选路、短时 probe 重试、有界 visitor/owner 会话状态和 pending 队列、直连响应代理名/方向/`sk` 字段校验、带 TTL 且有容量上限的响应确认直连 peer、未确认失败短 TTL 且有容量上限跳过并停止 probe 重试、fallback 后清理 pending 以抑制迟到 ack 重复投递、owner 响应 peer 刷新和服务端中继 fallback
- NAT 打洞控制器、按事务/代理名隔离的服务端 NAT 会话及候选查询/删除、按 `sk` 隔离的 peer 注册与候选匹配、事务表容量上限、同一事务多 peer 候选有界保留、按 peer 过期清理与客户端断线主动剪枝、匹配前控制连接存活校验与失效候选重选、候选局部过期过滤、全局清理节流、local candidate 数量限制、通配绑定地址的路由本地 IP 候选推导、控制通道候选地址交换、双端匹配通知、客户端同 key 并发 NAT 注册合并与有界 waiter fan-out、按事务/代理名/角色隔离的异步通知缓存、带 TTL 且有容量上限的异步通知缓存、XTCP/SUDP owner 候选周期刷新、XTCP/SUDP waiting 后短暂等待异步匹配与短 TTL 去重、XTCP/SUDP 分组直连注册、XTCP owner 本地服务确认后接受直连、SUDP owner 主动 punch 短时重试、不可用直连候选过滤、按地址可达性优先且有数量上限的候选优选、XTCP 候选竞速与慢候选取消、XTCP 成功直连 peer 短 TTL 且有容量上限复用、XTCP 失败候选短 TTL 且有容量上限跳过和 SUDP 候选探测/重试/失败短 TTL 且有容量上限跳过与重试降噪
- NAT 双端匹配会在返回候选前确认对端控制通知写入成功；通知失败的 peer 会立即剪枝并在同事务、代理和 `sk` 范围内重选
- 可选 SUDP STUN 映射发现，owner/visitor 均从实际直连 UDP socket 探测并按 socket 缓存公网候选，复用会话级 DNS 解析且在固定总超时内有界重传
...
- UDP：已实现基础请求/响应转发、本地 NAT 会话复用、双向包批量转发、分组包批量转发、非阻塞批处理队列、截止时间批窗口、批处理目标缓存、批路径预分配、服务端批处理缓冲复用、datagram 插件会话缓存清理节流和分组负载均衡；更深入的包级优化仍待完善。
...
- SUDP STUN：客户端配置 `[transport].natHoleStunServer` 后，owner 和 visitor 会用各自实际直连 UDP socket 发起 RFC 5389 Binding 探测，将校验事务 ID 后得到的 `XOR-MAPPED-ADDRESS` 置于本地候选之前，并按 socket 复用候选；STUN DNS 结果在当前客户端会话内复用，请求会在 500ms 总预算内有界重传，未配置时不会产生额外网络请求。
...
- STCP/XTCP private visitor protocols with `sk` authentication and local visitor listeners; XTCP connects directly when the peer is reachable and the handshake response proxy name matches, and falls back to server relay otherwise
- STCP/XTCP `group`/`groupKey` load balancing, allowing visitors to target a private service group
- SUDP private visitor protocol with `sk` authentication, `group`/`groupKey` load balancing, direct UDP forwarding by proxy name or group name when the peer is reachable, probe-based path selection, short probe retries, bounded visitor/owner session state and pending queue, direct-response proxy-name/direction/`sk`-field validation, TTL-capped and bounded response-confirmed direct peer caching, short TTL and bounded skip after unconfirmed direct fallback with probe retry suppression, pending cleanup after fallback to suppress late-ack duplicate delivery, owner response peer refresh, and server-relay fallback
- NAT hole punching controller, server-side NAT sessions plus candidate lookup/removal scoped by transaction/proxy, `sk`-isolated peer registration and candidate matching, bounded session table, bounded multiple peers per transaction with per-peer expiry, disconnect pruning, pre-match control liveness checks with stale-candidate reselection, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, route-derived local candidates for wildcard binds, control-channel candidate exchange, two-sided match notification, same-key concurrent register coalescing plus bounded waiter fan-out, transaction/proxy/role-scoped async notifications, client-side TTL-capped and bounded async notification caching, periodic XTCP/SUDP owner candidate refresh, short XTCP/SUDP wait for async matches after `waiting`, short TTL deduplication for repeated waiting lookups, XTCP/SUDP group direct registration, XTCP owner-side local-service validation before direct acceptance, SUDP owner-side active punching with short retries, unusable direct-candidate filtering, reachability-first and bounded candidate preference, XTCP candidate racing with slower-candidate cancellation, TTL-capped and bounded successful XTCP direct peer reuse, short TTL and bounded skip for failed XTCP candidates, and SUDP candidate probing/retry plus short TTL and bounded skip and retry suppression after failed unconfirmed direct sessions
- Two-sided NAT matching confirms that the peer control notification was written before returning a candidate; failed peers are pruned and reselected within the same transaction, proxy, and `sk` scope
- Optional SUDP STUN mapping discovery from the actual owner/visitor direct UDP sockets, with public candidates cached per socket, session-scoped DNS reuse, and bounded retries within a fixed total timeout
...
- UDP: basic request/response forwarding, local NAT session reuse, bidirectional packet batching, grouped packet batching, nonblocking batch queues, deadline-based batch windows, batch destination caching, batch-path preallocation, server-side batch buffer reuse, and group load balancing implemented; deeper packet-level optimizations remain.
...
- SUDP STUN: when `[transport].natHoleStunServer` is configured, owners and visitors send RFC 5389 Binding requests from their actual direct UDP sockets, prioritize the transaction-validated `XOR-MAPPED-ADDRESS` over local candidates, and reuse candidates per socket; STUN DNS results are reused for the client session and requests retry within a 500 ms total budget, while no extra network request is made when the option is unset.
