...
- SUDP 私有 visitor 协议，支持 `sk` 密钥校验、`group`/`groupKey` 分组负载均衡、peer 可达时按代理名或分组名直连 UDP 转发、probe 选路、短时 probe 重试、有界 pending 队列、带 TTL 的响应确认直连 peer、未确认失败短 TTL 跳过并停止 probe 重试、fallback 后清理 pending 以抑制迟到 ack 重复投递、owner 响应 peer 刷新和服务端中继 fallback
- NAT 打洞控制器、同一事务多 peer 候选保留、按 peer 过期清理与客户端断线主动剪枝、候选局部过期过滤、全局清理节流、local candidate 数量限制、控制通道候选地址交换、双端匹配通知、客户端同 key 并发 NAT 注册合并与 waiter fan-out、带 TTL 且有容量上限的异步通知缓存、XTCP/SUDP owner 候选周期刷新、XTCP/SUDP waiting 后短暂等待异步匹配与短 TTL 去重、XTCP/SUDP 分组直连注册、XTCP owner 本地服务确认后接受直连、SUDP owner 主动 punch 短时重试、不可用直连候选过滤、按地址可达性优先的候选优选、XTCP 候选竞速与慢候选取消、XTCP 成功直连 peer 短 TTL 复用、XTCP 失败候选短 TTL 跳过和 SUDP 候选探测/重试/失败短 TTL 跳过与重试降噪
- 通过 `transport.protocol` 支持 TCP、TCP stream mux、TLS、WebSocket、QUIC、KCP 控制/工作连接传输
...
- UDP：已实现基础请求/响应转发、本地 NAT 会话复用、双向包批量转发、分组包批量转发、批处理目标缓存、批路径预分配、datagram 插件会话缓存清理节流和分组负载均衡；更深入的包级优化仍待完善。
...
- 连接池：已通过 `poolCount` 实现预创建 work connection，合并高并发补池请求，并按等待者即时需求精确请求连接；TCP stream mux/QUIC 传输会在同一 client session 上复用 control/work stream。
- 带宽限制：已实现按代理限速。
...
- SUDP/XTCP/P2P：已实现 XTCP/SUDP 直连优先数据面、SUDP 私有服务分组负载均衡、XTCP/SUDP 按分组名直连注册与探测、XTCP/SUDP owner 候选周期刷新、NAT 控制器同一事务多 peer 候选保留、按 peer 过期清理与轮询、候选局部过期过滤、全局清理节流、local candidate 数量限制、XTCP/SUDP waiting 后短暂等待异步匹配与短 TTL 去重、同 key 并发 NAT 注册合并与 waiter fan-out、XTCP owner 本地服务确认后接受直连、SUDP probe 选路与短时重试、SUDP direct 有界 pending 队列、SUDP fallback 后 pending 清理、SUDP owner 主动 punch 短时重试、SUDP owner 响应 peer 刷新、XTCP 成功直连 peer 短 TTL 复用、SUDP 带 TTL 的响应确认直连 peer/未确认回退/失败短 TTL 跳过与 probe 重试降噪、服务端中继 fallback、NAT 双端匹配通知、客户端带 TTL 且有容量上限的异步通知缓存、不可用直连候选过滤、按地址可达性优先的候选优选、XTCP 候选竞速/慢候选取消和失败候选短 TTL 跳过；更完整的复杂 NAT 场景仍待验证和完善。
- Transport/热路径：已补 TLS、WebSocket、TCP stream mux、QUIC client session 复用、基础 TCP mux、work connection 补池合并、等待者按需请求、UDP 批处理缓存/批路径预分配和 datagram 插件会话清理节流；后续继续补更多连接池调优。
...
- pre-opened TCP work connection pools with `poolCount`, coalesced replenish requests under concurrent load, and demand-aware waiter requests
...
- SUDP private visitor protocol with `sk` authentication, `group`/`groupKey` load balancing, direct UDP forwarding by proxy name or group name when the peer is reachable, probe-based path selection, short probe retries, bounded pending queue, TTL-capped response-confirmed direct peer caching, short TTL skip after unconfirmed direct fallback with probe retry suppression, pending cleanup after fallback to suppress late-ack duplicate delivery, owner response peer refresh, and server-relay fallback
- NAT hole punching controller, multiple peers per transaction with per-peer expiry, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, control-channel candidate exchange, two-sided match notification, same-key concurrent register coalescing plus waiter fan-out, client-side TTL-capped and bounded async notification caching, periodic XTCP/SUDP owner candidate refresh, short XTCP/SUDP wait for async matches after `waiting`, short TTL deduplication for repeated waiting lookups, XTCP/SUDP group direct registration, XTCP owner-side local-service validation before direct acceptance, SUDP owner-side active punching with short retries, unusable direct-candidate filtering, reachability-first candidate preference, XTCP candidate racing with slower-candidate cancellation, TTL-capped successful XTCP direct peer reuse, short TTL skip for failed XTCP candidates, and SUDP candidate probing/retry plus short TTL skip and retry suppression after failed unconfirmed direct sessions
- TCP, TCP stream mux, TLS, WebSocket, QUIC, and KCP control/work transports via `transport.protocol`
...
- Connection pool: pre-opened work connections implemented through `poolCount`, with coalesced replenish requests, demand-aware waiter requests, and TCP stream mux/QUIC client-session reuse for control/work streams.
...
- SUDP/XTCP/P2P: XTCP/SUDP direct-first data paths, SUDP private-service group load balancing, XTCP/SUDP direct registration and probing by group name, periodic XTCP/SUDP owner candidate refresh, multiple peers per NAT transaction with per-peer expiry, candidate round-robin, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, same-key concurrent register coalescing plus waiter fan-out, short XTCP/SUDP wait for async matches after `waiting`, short TTL deduplication for repeated waiting lookups, XTCP owner-side local-service validation before direct acceptance, SUDP probe-based path selection with short retries, bounded SUDP direct pending queue, pending cleanup after fallback, SUDP owner-side active punching with short retries, SUDP owner response peer refresh, TTL-capped successful XTCP direct peer reuse, SUDP TTL-capped response-confirmed direct peer caching/unconfirmed fallback/failed direct skip with probe retry suppression, server-relay fallback, two-sided NAT match notification, client-side TTL-capped and bounded async notification caching, unusable direct-candidate filtering, reachability-first candidate preference, XTCP candidate racing with slower-candidate cancellation, and short TTL skip for failed XTCP candidates are implemented; fuller complex NAT scenarios still need broader validation and tuning.
- Transport/hot paths: TLS, WebSocket, TCP stream mux, QUIC client-session reuse, basic TCP mux, work-connection replenish coalescing, waiter demand-aware requests, UDP group batching, UDP batch-path caching/preallocation, and throttled datagram plugin-session pruning implemented; add more connection-pool tuning.
