...
- NAT 打洞控制器、控制通道候选地址交换、双端匹配通知、客户端异步通知缓存、XTCP waiting 后短暂等待异步匹配、XTCP/SUDP 分组直连注册、SUDP owner 主动 punch、候选优选、XTCP 候选竞速、SUDP 候选探测和重试
...
- SUDP/XTCP/P2P：已实现 XTCP/SUDP 直连优先数据面、SUDP 私有服务分组负载均衡、XTCP/SUDP 按分组名直连注册与探测、XTCP waiting 后短暂等待异步匹配、SUDP probe 选路与短时重试、SUDP owner 主动 punch、SUDP 直连确认/未确认回退、服务端中继 fallback、NAT 双端匹配通知、客户端异步通知缓存、候选优选、XTCP 候选竞速和 NAT 控制器基础；更完整的 NAT 打洞协商和复杂 NAT 场景仍待实现。
...
- NAT hole punching controller, control-channel candidate exchange, two-sided match notification, client-side async notification caching, short XTCP wait for async matches after `waiting`, XTCP/SUDP group direct registration, SUDP owner-side active punching, candidate preference, XTCP candidate racing, and SUDP candidate probing/retry
