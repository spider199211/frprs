...
- UDP：已实现基础请求/响应转发、本地 NAT 会话复用、双向包批量转发、分组包批量转发、批处理目标缓存、批路径预分配和分组负载均衡；更深入的包级优化仍待完善。
...
- Transport/热路径：已补 TLS、WebSocket、TCP stream mux、QUIC client session 复用、基础 TCP mux、work connection 补池合并、等待者按需请求、UDP 批处理缓存和批路径预分配；后续继续补更多连接池调优。
...
- Transport/hot paths: TLS, WebSocket, TCP stream mux, QUIC client-session reuse, basic TCP mux, work-connection replenish coalescing, waiter demand-aware requests, UDP group batching, UDP batch-path caching, and batch-path preallocation implemented; add more connection-pool tuning.
