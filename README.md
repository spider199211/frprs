...
- NAT 打洞控制器、同一事务多 peer 候选保留、按 peer 过期清理与客户端断线主动剪枝、候选局部过期过滤、全局清理节流、local candidate 数量限制、控制通道候选地址交换、双端匹配通知、客户端同 key 并发 waiter fan-out、带 TTL 且有容量上限的异步通知缓存、XTCP/SUDP owner 候选周期刷新、XTCP/SUDP waiting 后短暂等待异步匹配与短 TTL 去重、XTCP/SUDP 分组直连注册、XTCP owner 本地服务确认后接受直连、SUDP owner 主动 punch 短时重试、不可用直连候选过滤、按地址可达性优先的候选优选、XTCP 候选竞速与慢候选取消、XTCP 成功直连 peer 短 TTL 复用、XTCP 失败候选短 TTL 跳过和 SUDP 候选探测/重试/失败短 TTL 跳过与重试降噪
...
- NAT hole punching controller, multiple peers per transaction with per-peer expiry, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, control-channel candidate exchange, two-sided match notification, same-key concurrent waiter fan-out, client-side TTL-capped and bounded async notification caching, periodic XTCP/SUDP owner candidate refresh, short XTCP/SUDP wait for async matches after `waiting`, short TTL deduplication for repeated waiting lookups, XTCP/SUDP group direct registration, XTCP owner-side local-service validation before direct acceptance, SUDP owner-side active punching with short retries, unusable direct-candidate filtering, reachability-first candidate preference, XTCP candidate racing with slower-candidate cancellation, TTL-capped successful XTCP direct peer reuse, short TTL skip for failed XTCP candidates, and SUDP candidate probing/retry plus short TTL skip and retry suppression after failed unconfirmed direct sessions
