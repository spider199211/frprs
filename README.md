...
- UDP：已实现基础请求/响应转发、本地 NAT 会话复用、双向包批量转发、分组包批量转发、批处理目标缓存、批路径预分配、服务端批处理缓冲复用、datagram 插件会话缓存清理节流和分组负载均衡；更深入的包级优化仍待完善。
...
- UDP: basic request/response forwarding, local NAT session reuse, bidirectional packet batching, grouped packet batching, batch destination caching, batch-path preallocation, server-side batch buffer reuse, and group load balancing implemented; deeper packet-level optimizations remain.
