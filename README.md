...
- NAT 打洞控制器、控制通道候选地址交换、候选优选、XTCP 候选竞速和 SUDP 候选探测
...
- SUDP/XTCP/P2P：已实现 XTCP/SUDP 直连优先数据面、SUDP probe 选路、SUDP 直连确认/未确认回退、服务端中继 fallback、候选优选、XTCP 候选竞速和 NAT 控制器基础；更完整的 NAT 打洞协商和复杂 NAT 场景仍待实现。
...
- NAT hole punching controller, control-channel candidate exchange, candidate preference, XTCP candidate racing, and SUDP candidate probing
...
- SUDP/XTCP/P2P: XTCP/SUDP direct-first data paths, SUDP probe-based path selection, SUDP direct confirmation/unconfirmed fallback, server-relay fallback, candidate preference, XTCP candidate racing, and the NAT controller foundation are implemented; fuller NAT-hole negotiation and complex NAT scenarios remain.
