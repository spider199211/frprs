...
- UDP 本地会话复用、双向批量转发、分组批量转发、非阻塞批处理队列、批处理热路径缓存和 `group`/`groupKey` 分组负载均衡
...
- UDP：已实现基础请求/响应转发、本地 NAT 会话复用、双向包批量转发、分组包批量转发、非阻塞批处理队列、批处理目标缓存、批路径预分配、服务端批处理缓冲复用、datagram 插件会话缓存清理节流和分组负载均衡；更深入的包级优化仍待完善。
...
- Transport/热路径：已补 TLS、WebSocket、TCP stream mux、QUIC client session 复用、基础 TCP mux、work connection 补池合并、等待者按需请求、客户端 work connection 建连限流、UDP 非阻塞批处理队列、UDP 批处理缓存/批路径预分配、服务端批处理缓冲复用和 datagram 插件会话清理节流；后续继续补更多连接池调优。
...
- UDP local session reuse, bidirectional batching, nonblocking batch queues, batch-path caching, and `group`/`groupKey` load balancing
...
- NAT hole punching controller, bounded session table, bounded multiple peers per transaction with per-peer expiry, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, control-channel candidate exchange, two-sided match notification, same-key concurrent register coalescing plus bounded waiter fan-out, transaction/proxy/role-scoped async notifications, client-side TTL-capped and bounded async notification caching, periodic XTCP/SUDP owner candidate refresh, short XTCP/SUDP wait for async matches after `waiting`, short TTL deduplication for repeated waiting lookups, XTCP/SUDP group direct registration, XTCP owner-side local-service validation before direct acceptance, SUDP owner-side active punching with short retries, unusable direct-candidate filtering, reachability-first and bounded candidate preference, XTCP candidate racing with slower-candidate cancellation, TTL-capped and bounded successful XTCP direct peer reuse, short TTL and bounded skip for failed XTCP candidates, and SUDP candidate probing/retry plus short TTL and bounded skip and retry suppression after failed unconfirmed direct sessions
...
- UDP: basic request/response forwarding, local NAT session reuse, bidirectional packet batching, grouped packet batching, nonblocking batch queues, batch destination caching, batch-path preallocation, server-side batch buffer reuse, and group load balancing implemented; deeper packet-level optimizations remain.
