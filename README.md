...
- SUDP 私有 visitor 协议，支持 `sk` 密钥校验、peer 可达时直连 UDP 转发、直连确认和服务端中继 fallback
- NAT 打洞控制器、控制通道候选地址交换、候选优选和 XTCP 候选竞速
...
- SUDP/XTCP/P2P：已实现 XTCP/SUDP 直连优先数据面、SUDP 直连确认/未确认回退、服务端中继 fallback、候选优选、XTCP 候选竞速和 NAT 控制器基础；更完整的 NAT 打洞协商和复杂 NAT 场景仍待实现。
...
- SUDP private visitor protocol with `sk` authentication, direct UDP forwarding when the peer is reachable, direct confirmation, and server-relay fallback
- NAT hole punching controller, control-channel candidate exchange, candidate preference, and XTCP candidate racing
...
- SUDP/XTCP/P2P: XTCP/SUDP direct-first data paths, SUDP direct confirmation/unconfirmed fallback, server-relay fallback, candidate preference, XTCP candidate racing, and the NAT controller foundation are implemented; fuller NAT-hole negotiation and complex NAT scenarios remain.
