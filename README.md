...
- 可选 SUDP STUN 映射发现，owner/visitor 均从实际直连 UDP socket 探测并按 socket 缓存公网候选
...
- SUDP STUN：客户端配置 `[transport].natHoleStunServer` 后，owner 和 visitor 会用各自实际直连 UDP socket 发起 RFC 5389 Binding 探测，将校验事务 ID 后得到的 `XOR-MAPPED-ADDRESS` 置于本地候选之前，并按 socket 复用候选；未配置时不会产生额外网络请求。
...
- Optional SUDP STUN mapping discovery from the actual owner/visitor direct UDP sockets, with public candidates cached per socket
