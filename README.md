...
- NAT 打洞控制器、同一事务多 peer 候选保留与按 peer 过期清理、控制通道候选地址交换、双端匹配通知、客户端带 TTL 的异步通知缓存、XTCP/SUDP waiting 后短暂等待异步匹配、XTCP/SUDP 分组直连注册、SUDP owner 主动 punch 短时重试、候选优选、XTCP 候选竞速、SUDP 候选探测和重试
...
- SUDP/XTCP/P2P：已实现 XTCP/SUDP 直连优先数据面、SUDP 私有服务分组负载均衡、XTCP/SUDP 按分组名直连注册与探测、NAT 控制器同一事务多 peer 候选保留、按 peer 过期清理与轮询、XTCP/SUDP waiting 后短暂等待异步匹配、SUDP probe 选路与短时重试、SUDP owner 主动 punch 短时重试、SUDP 直连确认/未确认回退、服务端中继 fallback、NAT 双端匹配通知、客户端带 TTL 的异步通知缓存、候选优选和 XTCP 候选竞速；更完整的复杂 NAT 场景仍待验证和完善。
...
- NAT hole punching controller, multiple peers per transaction with per-peer expiry, control-channel candidate exchange, two-sided match notification, client-side TTL-capped async notification caching, short XTCP/SUDP wait for async matches after `waiting`, XTCP/SUDP group direct registration, SUDP owner-side active punching with short retries, candidate preference, XTCP candidate racing, and SUDP candidate probing/retry
...
- SUDP/XTCP/P2P: XTCP/SUDP direct-first data paths, SUDP private-service group load balancing, XTCP/SUDP direct registration and probing by group name, multiple peers per NAT transaction with per-peer expiry and candidate round-robin, short XTCP/SUDP wait for async matches after `waiting`, SUDP probe-based path selection with short retries, SUDP owner-side active punching with short retries, SUDP direct confirmation/unconfirmed fallback, server-relay fallback, two-sided NAT match notification, client-side TTL-capped async notification caching, candidate preference, and XTCP candidate racing are implemented; fuller complex NAT scenarios still need broader validation and tuning.
