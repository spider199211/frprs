...
| XTCP / P2P | Implemented | NAT controller, multiple peers per transaction with per-peer expiry and disconnect pruning, candidate exchange, TTL-capped async notifications, periodic owner candidate refresh, XTCP direct data plane with candidate racing, owner local-service validation, and short TTL failure skip, SUDP direct data plane, probing/retry, owner punch retry, SUDP owner response peer refresh, TTL-capped response-confirmed SUDP direct peers, and fallback are implemented. Complex NAT scenarios still need broader validation and tuning. |
