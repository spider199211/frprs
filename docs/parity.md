...
| XTCP / P2P | Implemented | NAT controller, multiple peers per transaction with per-peer expiry, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, and disconnect pruning, candidate exchange, TTL-capped and bounded async notifications, periodic owner candidate refresh, short TTL deduplication for repeated waiting lookups, unusable direct-candidate filtering, reachability-first candidate preference, XTCP direct data plane with candidate racing and slower-candidate cancellation, owner local-service validation, TTL-capped successful XTCP direct peer reuse, and short TTL failure skip, SUDP direct data plane, bounded pending queue, fallback pending cleanup, probing/retry with failure suppression, owner punch retry, SUDP owner response peer refresh, TTL-capped response-confirmed SUDP direct peers, unconfirmed fallback, and short TTL failed-direct skip are implemented. Complex NAT scenarios still need broader validation and tuning. |
...
- NAT hole controller candidate exchange, owner candidate refresh, stale peer expiry, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, bounded async notification caching, disconnected peer pruning, repeated waiting lookup deduplication, unusable direct-candidate filtering, reachability-first candidate preference, XTCP successful direct peer reuse, slower-candidate cancellation, and failed-candidate skip, SUDP failed-direct skip and probe retry suppression, XTCP owner local-service validation, and stale client notification cache expiry.
