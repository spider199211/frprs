...
| XTCP / P2P | Implemented | NAT controller, multiple peers per transaction with per-peer expiry and disconnect pruning, candidate exchange, TTL-capped async notifications, periodic owner candidate refresh, unusable direct-candidate filtering, reachability-first candidate preference, XTCP direct data plane with candidate racing, owner local-service validation, and short TTL failure skip, SUDP direct data plane, bounded pending queue, probing/retry with failure suppression, owner punch retry, SUDP owner response peer refresh, TTL-capped response-confirmed SUDP direct peers, unconfirmed fallback, and short TTL failed-direct skip are implemented. Complex NAT scenarios still need broader validation and tuning. |
...
- NAT hole controller candidate exchange, owner candidate refresh, stale peer expiry, disconnected peer pruning, unusable direct-candidate filtering, reachability-first candidate preference, XTCP failed-candidate skip, SUDP failed-direct skip and probe retry suppression, XTCP owner local-service validation, and stale client notification cache expiry.
