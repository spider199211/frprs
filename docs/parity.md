...
| Connection pool | Implemented | `poolCount` pre-opens work connections, preallocates and bounds the idle queue by pool size while admitting server-reserved waiter connections, coalesces replenishment, requests only immediate waiter demand, bounds client-side work-connection dialing, and reuses TCP stream mux/QUIC client sessions for control/work streams. |
...
| XTCP / P2P | Implemented | NAT controller, server-side sessions plus candidate lookup/removal scoped by transaction/proxy, `sk`-isolated peer registration and candidate matching, bounded session table, bounded multiple peers per transaction with per-peer expiry, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, route-derived local candidates for wildcard binds, disconnect pruning, pre-match control liveness checks with stale-candidate reselection, candidate exchange, same-key concurrent register coalescing plus bounded waiter fan-out, transaction/proxy/role-scoped TTL-capped and bounded async notifications, periodic owner candidate refresh, short TTL deduplication for repeated waiting lookups, unusable direct-candidate filtering, reachability-first and bounded candidate preference, XTCP direct data plane with candidate racing and slower-candidate cancellation, owner local-service validation, direct handshake response proxy-name validation, TTL-capped and bounded successful XTCP direct peer reuse, and short TTL and bounded failure skip, SUDP direct data plane, optional RFC 5389 STUN mapping discovery with per-socket candidate reuse, session-scoped DNS caching, and bounded request retries for owner/visitor direct sockets, bounded visitor/owner sessions and pending queue, direct-response proxy-name/direction/`sk`-field validation, fallback pending cleanup, probing/retry with failure suppression, owner punch retry, SUDP owner response peer refresh, TTL-capped and bounded response-confirmed SUDP direct peers, unconfirmed fallback, and short TTL and bounded failed-direct skip are implemented. Complex NAT scenarios still need broader validation and tuning. |
...
- TCP proxy with bounded idle work-connection pools, reserved waiter capacity, bounded client-side work-connection dialing, and TCP stream mux transport coverage.
...
- NAT hole controller candidate exchange, owner candidate refresh, stale peer expiry, transaction/proxy session isolation, session table limits, per-role peer limits, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, route-derived local candidates for wildcard binds, same-key concurrent register coalescing plus bounded waiter fan-out, proxy-scoped bounded async notification caching, disconnected peer pruning, repeated waiting lookup deduplication, unusable direct-candidate filtering, reachability-first and bounded candidate preference, XTCP successful direct peer reuse, XTCP direct response proxy-name validation, slower-candidate cancellation, and failed-candidate skip, SUDP direct-response proxy-name/direction/`sk`-field validation, SUDP failed-direct skip and probe retry suppression, XTCP owner local-service validation, and stale client notification cache expiry.
- Secret-isolated XTCP/SUDP NAT registration and candidate matching, including legacy secretless-message decoding.
- Notification-confirmed NAT matching with immediate failed-peer pruning and scoped reselection.
- Configured SUDP owner/visitor STUN mapping discovery with per-socket candidate reuse, session-scoped DNS caching, bounded retries, IPv4/IPv6 XOR-MAPPED-ADDRESS parsing, and transaction validation.
