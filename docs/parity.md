...
| XTCP / P2P | Implemented | NAT controller, multiple peers per transaction with per-peer expiry, candidate exchange, async notifications, XTCP direct data plane, SUDP direct data plane, probing/retry, owner punch retry, and fallback are implemented. Complex NAT scenarios still need broader validation and tuning. |
...
- NAT hole controller candidate exchange and stale peer expiry.
