...
- NAT hole controller candidate exchange, owner candidate refresh, stale peer expiry, per-role peer limits, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, same-key concurrent register coalescing plus bounded waiter fan-out, proxy-scoped bounded async notification caching, disconnected peer pruning, repeated waiting lookup deduplication, unusable direct-candidate filtering, reachability-first and bounded candidate preference, XTCP successful direct peer reuse, slower-candidate cancellation, and failed-candidate skip, SUDP direct-response proxy-name validation, SUDP failed-direct skip and probe retry suppression, XTCP owner local-service validation, and stale client notification cache expiry.
