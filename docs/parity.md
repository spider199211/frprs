...
| XTCP / P2P | Implemented | NAT controller, multiple peers per transaction with per-peer expiry, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, and disconnect pruning, candidate exchange, same-key concurrent register coalescing plus bounded waiter fan-out, TTL-capped and bounded async notifications, periodic owner candidate refresh, short TTL deduplication for repeated waiting lookups, unusable direct-candidate filtering, reachability-first candidate preference, XTCP direct data plane with candidate racing and slower-candidate cancellation, owner local-service validation, TTL-capped and bounded successful XTCP direct peer reuse, and short TTL and bounded failure skip, SUDP direct data plane, bounded pending queue, direct-response proxy-name validation, fallback pending cleanup, probing/retry with failure suppression, owner punch retry, SUDP owner response peer refresh, TTL-capped and bounded response-confirmed SUDP direct peers, unconfirmed fallback, and short TTL and bounded failed-direct skip are implemented. Complex NAT scenarios still need broader validation and tuning. |
...
- NAT hole controller candidate exchange, owner candidate refresh, stale peer expiry, local stale-candidate filtering, throttled global cleanup, local-candidate count limiting, same-key concurrent register coalescing plus bounded waiter fan-out, bounded async notification caching, disconnected peer pruning, repeated waiting lookup deduplication, unusable direct-candidate filtering, reachability-first candidate preference, XTCP successful direct peer reuse, slower-candidate cancellation, and failed-candidate skip, SUDP direct-response proxy-name validation, SUDP failed-direct skip and probe retry suppression, XTCP owner local-service validation, and stale client notification cache expiry.
